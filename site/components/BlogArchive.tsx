...
// - xl+: sticky right rail with the full archive
// - below xl (laptops/tablets/phones): compact archive section that flows
//   after the article body, so no viewport misses the archive.
export function BlogArchive({ current }: { current?: string }) {
  const post = POSTS.find((p) => p.slug === current);
  return (
    <>
      <aside aria-label="Blog archive" className="hidden w-72 shrink-0 xl:block">
        <nav
                ...
                >
                  ...
                  <span
                    className={`text-sm leading-snug ${
                      p.slug === current ? 'text-blue-200' : 'text-slate-300 hover:text-blue-300'
                    }`}
                  >
                    {p.title}
                  </span>
      ...
      {/* Inline variant for viewports below xl — rendered by the flex parent
          after the article column; w-full so it wraps under the content. */}
      <section aria-label="Blog archive" className="mt-14 w-full max-w-3xl xl:hidden">
        {post?.tags?.length ? (
          <div className="mb-8">
            <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-blue-300/80">
              Filed under
          ...
          {POSTS.filter((p) => p.slug !== current)
            ...
            .map((p) => (
              <li key={p.slug}>
                <a
                  href={`/blog/${p.slug}`}
                  className="block h-full rounded border border-white/[0.06] bg-white/[0.02] p-4 transition-all hover:border-blue-500/40 hover:bg-blue-500/[0.04]"
                >
                  <time dateTime={p.date} className="block font-mono text-[10px] text-slate-500">
                    {p.date}
                  </time>
